...
class OrderService:
    ...
    def create_order_from_cart(self, buyer_id: int, store_id: int, shipping_address_id: int) -> Order:
        address = self.db.query(Address).filter(Address.id == shipping_address_id, Address.user_id == buyer_id).first()
        if not address:
        ...
        cart_items: List[CartItem] = self.db.query(CartItem).join(Product).filter(CartItem.user_id == buyer_id, Product.store_id == store_id).all()
        if not cart_items:
        ...
        for ci in cart_items:
            ...
            if product.status != ProductStatus.ACTIVE:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Product '{product.title}' is no longer available")
            if product.stock is not None and product.stock < ci.quantity:
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Insufficient stock for '{product.title}' (available: {product.stock})")
    ...
    def create_order_direct(self, buyer_id: int, data: OrderCreate) -> Order:
        ...
        order = Order(order_number=f'ORD-{uuid.uuid4().hex[:10].upper()}', buyer_id=buyer_id, store_id=data.store_id, status=OrderStatus.PLACED, total_amount=total, currency='THB', shipping_address_id=data.shipping_address_id, items=order_items)
        self.db.add(order)
    ...
    def list_store_orders(self, store_id: int, skip: int=0, limit: int=20, status_filter: Optional[OrderStatus]=None) -> Tuple[List[Order], int]:
        ...
        orders = q.options(joinedload(Order.items)).order_by(Order.created_at.desc()).offset(skip).limit(limit).all()
        return (orders, total)
    _SELLER_TRANSITIONS = {OrderStatus.PLACED: {OrderStatus.PACKING, OrderStatus.CANCELLED}, OrderStatus.PAID: {OrderStatus.PACKING, OrderStatus.CANCELLED}, OrderStatus.PACKING: {OrderStatus.SHIPPED}}
    _BUYER_TRANSITIONS = {OrderStatus.SHIPPED: {OrderStatus.DELIVERED}, OrderStatus.DELIVERED_PENDING_CONFIRM: {OrderStatus.DELIVERED}}

    def update_order_status(self, order_id: int, new_status: OrderStatus, changed_by_user_id: int, note: Optional[str]=None, is_seller: bool=False, is_buyer: bool=False) -> Order:
        ...
        if new_status not in allowed:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Cannot move order from '{order.status}' to '{new_status}' with your current role.")
