...
class StoreService:
    ...
    def create_store(self, store_data: StoreCreate, owner_id: int) -> Store:
        ...
        if existing_slug:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Store slug '{store_data.slug}' is already taken")
        existing_store = self.db.query(Store).filter(Store.owner_id == owner_id).first()
        if existing_store:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='You already have a store. Only one store per seller is allowed.')
        db_store = Store(owner_id=owner_id, slug=store_data.slug, name=store_data.name, description=store_data.description, logo_url=store_data.logo_url)
    ...
    def get_store_with_product_count(self, store_id: int) -> Optional[dict]:
        ...
        if not store:
            return None
        product_count = self.db.query(func.count(Product.id)).filter(Product.store_id == store_id, Product.status == ProductStatus.ACTIVE).scalar()
