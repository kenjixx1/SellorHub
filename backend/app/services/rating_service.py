...
class RatingService:
    """Service for store rating operations."""

    def __init__(self, db: Session):
        self.db = db

    def _require_completed_order(self, buyer_id: int, store_id: int):
        """Verify the buyer has at least one delivered order for this store."""
        order = (
            self.db.query(Order)
            .filter(
    ...
    def create_rating(self, buyer_id: int, data: RatingCreate) -> StoreRating:
        ...
        existing = (
            ...
            .filter(StoreRating.store_id == data.store_id, StoreRating.buyer_id == buyer_id)
            .first()
        )
        if existing:
            raise HTTPException(
    ...
    def update_rating(self, rating_id: int, buyer_id: int, data: RatingUpdate) -> StoreRating:
        rating = (
            self.db.query(StoreRating)
            .filter(StoreRating.id == rating_id, StoreRating.buyer_id == buyer_id)
            .first()
        )
        if not rating:
