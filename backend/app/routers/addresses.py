...
def list_addresses(current_user: User=Depends(get_current_user), db: Session=Depends(get_db)):
    return UserSystem(db).list_addresses(current_user.id)
...
def get_address(address_id: int, current_user: User=Depends(get_current_user), db: Session=Depends(get_db)):
    return UserSystem(db).get_address(address_id, current_user.id)
...
def create_address(data: AddressCreate, current_user: User=Depends(get_current_user), db: Session=Depends(get_db)):
    return UserSystem(db).create_address(current_user.id, data)
...
def update_address(address_id: int, data: AddressUpdate, current_user: User=Depends(get_current_user), db: Session=Depends(get_db)):
    return UserSystem(db).update_address(address_id, current_user.id, data)
...
def delete_address(address_id: int, current_user: User=Depends(get_current_user), db: Session=Depends(get_db)):
    UserSystem(db).delete_address(address_id, current_user.id)
