...
def validate_image_file(file: UploadFile) -> None:
    ...
    if file_ext not in settings.ALLOWED_IMAGE_EXTENSIONS:
        raise HTTPException(status_code=400, detail=f"Invalid file type. Allowed: {', '.join(settings.ALLOWED_IMAGE_EXTENSIONS)}")
...
async def save_upload_file_local(file: UploadFile, subfolder: str='products') -> str:
    ...
    if len(contents) > settings.MAX_FILE_SIZE:
        raise HTTPException(status_code=400, detail=f'File size exceeds maximum of {settings.MAX_FILE_SIZE / (1024 * 1024)}MB')
...
async def save_upload_file_s3(file: UploadFile, subfolder: str='products') -> str:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail='boto3 not installed. Install with: pip install boto3')
    ...
    if len(contents) > settings.MAX_FILE_SIZE:
        raise HTTPException(status_code=400, detail=f'File size exceeds maximum of {settings.MAX_FILE_SIZE / (1024 * 1024)}MB')
    try:
        s3_client.put_object(Bucket=settings.AWS_S3_BUCKET, Key=s3_key, Body=contents, ContentType=file.content_type)
...
async def save_upload_file_cloudinary(file: UploadFile, subfolder: str='products') -> str:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail='cloudinary not installed. Install with: pip install cloudinary')
    ...
    if len(contents) > settings.MAX_FILE_SIZE:
        raise HTTPException(status_code=400, detail=f'File size exceeds maximum of {settings.MAX_FILE_SIZE / (1024 * 1024)}MB')
    try:
        result = cloudinary.uploader.upload(contents, folder=subfolder, resource_type='image')
